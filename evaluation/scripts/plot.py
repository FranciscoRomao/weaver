...
def plot(config):
    ...
    if plot_property == 'compilation_time_two':
        data = []
    ...
    if plot_property == 'execution_time_two':
...
#
    ...
    if plot_property == 'analysis_plots':
        ...
        #superconducting complexity O(N**3) Sabre
        superconducting_complexity = [n_qubits[i]**3 for i in range(len(n_qubits))]

        #weaver complexity O(N**2)
        weaver_complexity = [n_qubits[i]**2 for i in range(len(n_qubits))]

        #geyser complexity O(K**3)
        #converted from K to N
        atomique_complexity = [n_qubits[i]**2.8 for i in range(len(n_qubits))]

        #[962, 3178, 4986, 6971, 10866, 18427]
        
        #5789.6487x**2−87825.2997x−103601.8515
        geyser_complexity = [5789.6487*n_qubits[i]**2 - 87825.2997*n_qubits[i] - 103601.8515 for i in range(len(n_qubits))]

        #x^5.8 * (1 / (1 + e^(-k*(x - x0))))
        #geyser_complexity = [3798105019.9745*1.0345**n_qubits[i] for i in range(len(n_qubits))]

        #geyser_complexity = [n_qubits[i]**5.8 * (1 / (1 + np.e**(-20*(n_qubits[i] - 5)))) for i in range(len(n_qubits))]
        ...
        #plt.text(10, 2, "Lower is better ↓", ha='center', va='bottom', fontsize=14, fontweight='bold', color='midnightblue')
        ...
        #ax[1].text(100, 10**8, "Lower is better ↓", ha='center', va='bottom', fontsize=14, fontweight='bold', color='midnightblue')

        #ax.set_ylim(ax.get_ylim()[0], ax.get_ylim()[1]*2)

        #ax[1].legend().set_bbox_to_anchor((0.45, 0.55))
        ...
        #ax[2].set_yticks([0, 0.02, 0.040, 0.060, 0.080, 0.1, 0.12])
