...
class ComposeCircuit:
    ...
    def compose_block(self, block):
        ...
        # block_ideal_unitary = (
        #     execute(block, Aer.get_backend("unitary_simulator"))
        #     .result()
        #     .get_unitary(block)
        # )
        block_ideal_unitary = qi.Operator(block).data
        ...
        while index < len(params):
            for qubit in range(comp_block.num_qubits):
                comp_block.u3(
                    params[index], params[index + 1], params[index + 2], qubit
                )
                index += 3
            ...
            if index < len(params) and block.num_qubits > 1:
                ...
                comp_block.append(
                    self.gates[block.num_qubits], self.confs[block.num_qubits][conf]
                )
                index += 1
