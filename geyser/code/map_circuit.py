...
class MapCircuit:
	...
	def __init__(self, circuit):
		...
		self.circuit = transpile(circuit,
						   basis_gates=['u3', 'cz'],
	...
	def generate_qubit_map(self):
		...
		for row in range(dimension_y):
			for col in range(dimension_x):
				...
				conditions = [col % dimension_x > 0,
							  row - 1 >= 0,
							  row - 1 >= 0 and col + 1 < dimension_x,
							  col + 1 < dimension_x and row * dimension_x + col + 1 < self.num_qubits,
							  (row + 1) * dimension_x + col < self.num_qubits,
							  col % dimension_x > 0 and (row + 1) * dimension_x + col - 1 < self.num_qubits]

				qubits_ids = [row * dimension_x + col - 1,
				...
				if conditions[3] and conditions[4]:
					blocks[block_id] = {}
					blocks[block_id]['qubits'] = [qubit_id, qubits_ids[3], qubits_ids[4]]
					block_id += 1
				if conditions[4] and conditions[5]:
					blocks[block_id] = {}
					blocks[block_id]['qubits'] = [qubit_id, qubits_ids[4], qubits_ids[5]]
					block_id += 1
		...
		for t_1 in blocks.keys():
			...
			for t_2 in blocks.keys():
				if t_2 != t_1:
					if any([qubit in blocks[t_1]['qubits'] for qubit in blocks[t_2]['qubits']]):
			...
			for t_2 in blocks.keys():
				if t_2 != t_1 and t_2 not in blocks[t_1]['exclusions_l1']:
					for t_3 in blocks[t_1]['exclusions_l1']:
						if any([qubit in blocks[t_3]['qubits'] for qubit in blocks[t_2]['qubits']]):
							blocks[t_1]['exclusions_l2'].add(t_2)

			blocks[t_1]['compatibles'] = set()
			for t_2 in blocks.keys():
				if t_2 != t_1 and t_2 not in blocks[t_1]['exclusions_l1'] and t_2 not in blocks[t_1]['exclusions_l2']:
