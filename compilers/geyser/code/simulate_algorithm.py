...
def main():
	...
	for i in range(len(ALGORITHMS)):
		...
		algo_file = './geyser/qasm/' + algo + '_original.qasm'
		...
		with open('./geyser/qasm/' + algo + '_mapped.qasm', 'w') as f:
			f.write(circuits['Mapped'].qasm())

		print('Circuit Mapped')

		with open('./geyser/qasm/' + algo + '_blocked.qasm', 'w') as f:
		...
		with open('./geyser/qasm/' + algo + '_composed.qasm', 'w') as f: 
		...
		for status, circuit in circuits.items():
			unitaries[status] = transpile(circuit, Aer.get_backend('unitary_simulator')).result().get_unitary(circuit)
