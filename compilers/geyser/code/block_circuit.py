...
class BlockCircuit:
	...
	def calculate_block_score(self, x):
		...
		layer = [self.qubit_evo[q][self.frontier[q]][0] for q in block if self.frontier[q] < self.lengths[q]]
		...
		while len(removed_qubits) < len(block):
			...
			for q in block:
				...
				if q not in removed_qubits and self.qubit_evo[q][positions[q]][0] == layer:

					if len(self.qubit_evo[q][positions[q]]) == 5 and \
					   (self.qubit_evo[q][positions[q]][3] not in block or \
					   self.qubit_evo[q][positions[q]][3] in removed_qubits):
	...
	def get_best_schedule(self, this_block, excluded_blocks, schedule, total_score):
		...
		if not self.blocks[this_block]['compatibles'] or \
		   all([t in excluded_blocks for t in self.blocks[this_block]['compatibles']]):
		...
		for t in self.blocks[this_block]['compatibles']:
			if t not in excluded_blocks:
				score, schedule = self.get_best_schedule(t, excl_blocks + [t], sched, total_score)
				if score > best_score:
					best_score = score
		...
		while self.frontier != self.lengths:
			...
			for block in best_schedule:
				...
				for q in self.blocks[block]['qubits']:
					...
					for p in range(self.frontier[q], block_positions[block][q]):
						...
						if self.qubit_evo[q][p][1] == 'u3':
							positions[self.qubit_evo[q][p][0]].append([self.qubit_evo[q][p][2], q])
						elif self.qubit_evo[q][p][4] == 'control':
							positions[self.qubit_evo[q][p][0]].append([self.qubit_evo[q][p][2], q, self.qubit_evo[q][p][3]])
						else:
				...
				if positions:
					...
					final_blocks.append(['c' + str(cycle) + 'b' + str(block), positions])
