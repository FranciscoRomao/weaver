...
from ..compiler.lookahead_compiler import LabeledOp, circuit_to_dag
...
def swap_num(circuit: qiskit.QuantumCircuit) -> int:
    ...
    for instruction in circuit:
        if isinstance(
            instruction[0], qiskit.circuit.library.standard_gates.swap.SwapGate
        ):
            num_swaps += 1
    return num_swaps
...
def swap_lengths(
    circuit: qiskit.QuantumCircuit, hardware: Hardware, physical_to_hardware
):
    """
    ...
    for instruction in circuit:
        if isinstance(
            ...
            distance = hardware.integer_distance(
                mapping[qubits[0].register], mapping[qubits[1].register]
            )
...
def digraph_to_time_steps(
    digraph: nx.DiGraph, id_to_instruction: Dict[int, qiskit.circuit.Instruction]
