...
class LookaheadCompilerConstrainedParallel(Compiler):
    start_node = LabeledOp([None, []], -1)
    final_op = LabeledOp([None, []], float("inf"))

    def __init__(
        self, interaction_model, hardware, percentage=1, spacing="as_needed"
    ) -> None:
        self.percentage_parallel = percentage
                    ...
                    if lo2 not in counted_los:
                        for q1 in lo2.qubits:
                            for q2 in lo2.qubits:
                                if q1 != q2:
                                    edges[frozenset({q1, q2})] += weighting_function(
                                        dist_dict[lo1][lo2]
                                    )
    ...
    def _dist_norm(self, A, B):
        if A == B:
            return 0
        if (
            self.hardware.distance(A, B)
            <= self.interaction_model.max_interaction_distance
        ):
            return 0
    ...
    def _initial_mapping(self, circuit, dist_dict, frontier, weighting_function):
        ...
        self.viable_nodes_sets = (
            self.hardware.get_non_overlapping_constrained_qubit_sets(
                len(circuit.qubits)
            )
        )
                ...
                if node not in physical_to_logical:
                    ...
                    for target in weight_to[q]:
                        if target in placed_qubits:
                            score += weight_to[q][target] * self._dist_norm(
                                logical_to_physical[target], node
                            )
                    if score < best_score:
    ...
    def _map_qubit(self, q, weight_to, l2p, p2l):
        ...
        for node in self.hardware.connectivity:
            if node not in self.viable_nodes_sets[0]:
                continue
            if node not in p2l:
                # If this node is unoccuped
                score = 0
                if q in weight_to:
                    for target in wedges[q]:
                        if target in placed_qubits:
                            score += wedges[q][target] * self._dist_norm(
                                l2p[target], node
                            )
                if score < best_score:
                    best_location, best_score = node, score
    ...
    def compile(
        self, circuit, lookahead_distance, weighting_function, extra_mapping_params=None
    ):
        ...
        all_paths = nx.algorithms.shortest_paths.all_pairs_shortest_path_length(
            dag, cutoff=lookahead_distance
        )
        ...
        l2p, p2l = self._initial_mapping(
            circuit, dist_dict, frontier, weighting_function
        )
        ...
        if l2p is None:
            print("no mapping")
            return None
        new_circuit = self._schedule_and_route(
            dag, l2p, p2l, lookahead_distance, weighting_function, dist_dict, frontier
        )
        qubit_mappings = defaultdict(dict)
        for j, constrained_set in enumerate(self.viable_nodes_sets):
            for i, qubit in enumerate(constrained_set):
                hq_qiskit = self.hardware.hardware_to_qiskit[
                    self.viable_nodes_sets[0][i]
                ]
                hq_qiskit_loc = self.hardware.hardware_to_qiskit[qubit]
                qubit_mappings[hq_qiskit][j] = hq_qiskit_loc

        first_set = constrained_set[0]
        increased_circuits = []
        self.included_constrained_sets = []
        number_parallel_at_once = int(
            np.ceil(len(self.viable_nodes_sets) * self.percentage_parallel)
        )
        if self.percentage_parallel == 0:
            number_parallel_at_once = 1
        separation_index = int(
            np.ceil(len(self.viable_nodes_sets) / number_parallel_at_once)
        )
        # print(number_parallel_at_once)
        # print(separation_index)
        current_index = 0
        while current_index < len(self.viable_nodes_sets):
            constrained_list = []
            included_sets = []
            end_loc = current_index + number_parallel_at_once
            if end_loc > len(self.viable_nodes_sets):
                end_loc = len(self.viable_nodes_sets)
            node_sets = set([i for i in range(current_index, end_loc)])
            increased_circuit = qiskit.QuantumCircuit(self.hardware.qiskit_qubits)
            for j, constrained_set in enumerate(self.viable_nodes_sets):
                if not j in node_sets:
                    continue
                constrained_list += list(constrained_set)
                included_sets.append(constrained_set)

            for operation in new_circuit:
                for j, constrained_set in enumerate(self.viable_nodes_sets):
                    if not j in node_sets:
                        continue
                    qs = [qubit_mappings[q][j] for q in operation[1]]
                    increased_circuit.append(operation[0], qs)
            current_index += number_parallel_at_once
            self.included_constrained_sets.append(
                (constrained_list, len(included_sets), included_sets)
            )
            increased_circuits.append(increased_circuit)
        # print(len(self.included_constrained_sets))
        return increased_circuits

    def _schedule_and_route(
        self, dag, l2p, p2l, lookahead_distance, weighting_function, dist_dict, frontier
    ):
        ...
        def _score_swaps_(qubits, wig, l2p, p2l, to_execute):
            cur_dist = {}
            literal_dist = {}
            psuedo_dists = {}
            for q1 in qubits:
            ...
            def _get_valid_swaps_(q):
                ...
                for node in self.hardware.qubits_in_radius(
                    pq, self.interaction_model.max_d()
                ):
                    dist = 0
                    psuedo_dist = 0
                    if node in pqs:
                    ...
                    if dist < literal_dist[q]:
                        if self.interaction_model.valid_interaction_constrained(
                            [node, pq], [[l2p[q] for q in o.qubits] for o in to_execute]
                        ):
                            locs.append(node)
                return locs
        ...
        while len(frontier) > 0:
            ...
            for op in frontier:
                ...
                for q in op.qubits:
                    if q not in l2p:
                        if weight_to is None:
                            wedges = self.build_interaction_edges(
                                dist_dict, frontier, weighting_function
                            )
                ...
                if len(op.qubits) == 1:
                    ...
                    if self.interaction_model.valid_interaction_distance(
            ...
            if len(requires_swaps) > 0:
                ...
                wedges_no_exec = self.build_interaction_edges(
                    dist_dict, frontier_no_exec, weighting_function
                )
                ...
                sorted_scores_keys = sorted(
                    all_scores.keys(), key=lambda x: all_scores[x][0], reverse=True
                )
            ...
            for op in to_execute:
                if isinstance(op, SwapOp):
                    ...
                    if l1 is not None and l2 is not None:
                        # Atomic swap
                        p2l[op.qubits[1]], p2l[op.qubits[0]] = (
                            p2l[op.qubits[0]],
                            p2l[op.qubits[1]],
                        )
