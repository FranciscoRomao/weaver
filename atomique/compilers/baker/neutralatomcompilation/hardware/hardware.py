...
class HardwareQubit:
    ...
    def __init__(
        self, hardware_tuple: Tuple[int], qiskit_qubit: qiskit.circuit.QuantumRegister
    ):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for dim in range(num_dimensions):
            self.dimension_info[dim] = DimensionData(
                dimensions_length[dim], dimensions_spacing[dim]
            )
        ...
        for index, qubit1 in enumerate(self.hardware_qubits):
            for qubit2 in self.hardware_qubits[index:]:
                if qubit1 == qubit2:
                    ...
                    squared_sum = sum(
                        [
                            (qubit1.dim_loc(i) - qubit2.dim_loc(i)) ** 2
                    ...
                    manhattan_distance = sum(
                        [
                            abs(qubit1.dim_loc(i) - qubit2.dim_loc(i))
                            for i in range(self.num_dimensions)
                        ]
                    )
    ...
    def get_constrained_qubit_sets(self, number_qubits, method="wide"):
        ...
        while curr_first + dims[0] <= self.dimension_info[0].size:
            ...
            while curr_second + dims[1] <= self.dimension_info[1].size:
                ...
                if (
                    curr_second + dims[1] >= self.dimension_info[1].size
                    and not break_out_inner
                ):
            ...
            curr_first += dims[0]
            if (
                curr_first + dims[0] > self.dimension_info[0].size
                and not break_out_outer
            ):
                curr_first = self.dimension_info[0].size - dims[0]
                break_out_outer = True
        return qubit_sets

    def get_non_overlapping_constrained_qubit_sets(
        self, number_qubits, method="wide", spacing=0
    ):
        ...
        if self.dimension_info[0].size > self.dimension_info[1].size:
            if (
                np.floor(self.dimension_info[0].size / satisified[1])
                * np.floor(self.dimension_info[1].size / satisified[0])
                >= number_qubits
            ):
            ...
            if (
                np.floor(self.dimension_info[1].size / satisified[1])
                * np.floor(self.dimension_info[0].size / satisified[0])
                >= number_qubits
            ):
    ...
    def qubits_in_radius(self, q: HardwareQubit, r: float) -> List[HardwareQubit]:
        ...
        for qubit in self.distances[q]:
            if self.distances[q][qubit] <= r:
                if qubit not in self.active_holes:
                    in_radius.append(qubit)
