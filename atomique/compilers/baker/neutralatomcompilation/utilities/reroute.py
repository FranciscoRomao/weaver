...
class HoleHandler:
    ...
    def shift(self, location, used_qubits, dim, direct, strat="short"):
        ...
        for i in range(start_loc, end_loc, -1 * direct):
            ...
            while (
                self.hardware.hardware_loc_to_obj[tuple(to_shift_from)]
                in self.active_holes
            ):
                curr_loc += -1 * direct
                to_shift_from[dimension] = curr_loc
            to_shift_from_hw_qubit = self.hh_mapping_reverse[
                self.hardware.hardware_loc_to_obj[tuple(to_shift_from)]
            ]
            if strat == "short":
    ...
    def im_shift(self, qiskit_qubits, hole, stategy="space"):
        ...
        while found_node is None and len(frontier) > 0:
            ...
            for f in frontier:
                visited.add(f)
                ns = sorted(
                    list(self.interaction_graph.neighbors(f)),
                    key=lambda x: self.hardware.distance(f, x),
                )
                ...
                for n in ns:
                    ...
                    if (
                        currently_here not in qiskit_qubits
                        and currently_here not in self.active_holes
                    ):
                        found_node = n
                        break
    ...
    def naive_shift(self, qiskit_qubits, hole, stategy="space"):
        ...
        for dimension, l in enumerate(location):
            ...
            for i in range(dim_start, location[dimension] + 1):
                ...
                if (
                    hw_qubit not in self.active_holes
                    and self.hh_mapping_reverse[hw_qubit] not in qiskit_qubits
                ):
                    all_holes = False
    ...
    def fail_reroute(self, circuit) -> bool:
        for instruction in circuit:
            ...
            try:
                ...
                print(
                    instruction[1], [self.qh_mapping[i] for i in instruction[1]], qubits
                )
                raise e
        return True
            ...
            if self.interaction_model.valid_interaction_distance(qubits):
                ...
                try:
                    path = nx.dijkstra_path(
                        self.interaction_graph, qubits[0], qubits[1]
                    )
                ...
                for index, p1 in enumerate(path[:-2]):
                    p2 = path[index + 1]
                    self.qubits_swaps_added_on[
                        self.hh_mapping_reverse[p1], self.hh_mapping_reverse[p2]
                    ] += 2
    ...
    def reroute_with_holes(
        ...
        # print(qiskit_qubits)
        if len(qiskit_qubits) > len(self.hardware.hardware_qubits) - len(
            self.active_holes
        ) - len(holes):
            return None, "size"
        ...
        if route_strategy == ReRouteStrategy.Fail:
            ...
            if works:
                ...
                return None, "routing, no insertion"
