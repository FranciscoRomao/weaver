...
class InteractionModel:
    ...
    def valid_interaction_constrained(
        self, qs: List[Tuple[int, int]], constraints: Iterable[List[Tuple[int, int]]]
    ) -> bool:
        """
        ...
        for c in constraints:
            ...
            if len(c) > 1:
                max_dist_pairwise = max(
                    self.hardware.distance(q1, q2) for q1 in c for q2 in c if q1 != q2
                )
            else:
                max_dist_pairwise = 0
            ...
            assert (
                max_dist_pairwise <= self.max_d()
            ), "Illegal qubit set in constraints."
        ...
        max_dist_qs = max(
            self.hardware.distance(q1, q2) for q1 in qs for q2 in qs if q1 != q2
        )
    ...
    def valid_interaction_distance(self, qs: List[Tuple[int, int]]) -> bool:
        """
