...
def maxDegree(
    list_gate_qubits: Sequence[Sequence[int]], count_program_qubit: int
) -> int:
...
class DPQA:
            ...
            for qq in range(self.n_q):
                for s in range(num_stage - 1):
                    if q != qq:
                        (self.dpqa).add(
                            Implies(
                                And(a[q][s], a[qq][s], c[q][s] < c[qq][s]),
                                x[q][s + 1] <= x[qq][s + 1],
                            )
                        )
                        (self.dpqa).add(
                            Implies(
                                And(a[q][s], a[qq][s], r[q][s] < r[qq][s]),
                                y[q][s + 1] <= y[qq][s + 1],
                            )
                        )
    ...
    def solver_init(self, num_stage: int = 2):
        ...
        # variables
        a = [[Bool(f"a_q{q}_t{t}") for t in range(num_stage)] for q in range(self.n_q)]
        # for col and row, the data does not matter if atom in SLM
        c = [[Int(f"c_q{q}_t{t}") for t in range(num_stage)] for q in range(self.n_q)]
        r = [[Int(f"r_q{q}_t{t}") for t in range(num_stage)] for q in range(self.n_q)]
        x = [[Int(f"x_q{q}_t{t}") for t in range(num_stage)] for q in range(self.n_q)]
        y = [[Int(f"y_q{q}_t{t}") for t in range(num_stage)] for q in range(self.n_q)]
